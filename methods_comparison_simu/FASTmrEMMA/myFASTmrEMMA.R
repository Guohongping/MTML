myFASTmrEMMA<-function(gen,phe,genRaw,kk,psmatrix,svpal,svmlod,Genformat,Likelihood,CLO){
    ...
    ebayes_EM<-function(x,z,y)
{
  ...
  return(list(u=u,sigma2=sigma2,wang=wang))
    ...
    likelihood<-function(xxn,xxx,yn,bbo)  
    {
      ...
      if(at1>0.5)
        ad<-cbind(xxn,xxx[,ww1])  #cbind将向量或矩阵拼接在一起,xxn是Q1，xxx[,ww1]是Q2
      else
        ad<-xxn
      if(abs(min(eigen(crossprod(ad,ad))$values))<1e-6)  #crossprod是指内积，solve是求逆矩阵
        bb<-solve(crossprod(ad,ad)+diag(ncol(ad))*0.01)%*%crossprod(ad,yn)
      else
        bb<-solve(crossprod(ad,ad))%*%crossprod(ad,yn)
      vv1<-as.numeric(crossprod((yn-ad%*%bb),(yn-ad%*%bb))/ns); #ad%*%bb是均值X\beta，vv1是yn方差
      ll1<-sum(log(abs(multinormal(yn,ad%*%bb,vv1))))  #对数似然函数
      sub<-1:ncol(ad);
      if(at1>0.5)
      {
        for(i in 1:at1)
        {
          ...
          lod[ww1[i]]<--2.0*(ll0-ll1)/(2.0*log(10))#LOD统计量的定义
        }
      }
      return (lod)#LOD统计量的定义
    }
	
	#没有标记基因时，估计\lambda(g)、EMMA算法
	#似然情况下矩阵特征值分解  （2）
    ...
    #likelihood求特征值特征向量（Z是空）
    emma.eigen.L.wo.Z <- function(K) {
      eig <- eigen(K,symmetric=TRUE)  #symmetric是否是对称矩阵
      return(list(values=eig$values,vectors=eig$vectors))
    }
    #likelihood（Z不是空）
    emma.eigen.L.w.Z <- function(Z,K,complete=TRUE) {
      if ( complete == FALSE ) {
        vids <- colSums(Z)>0  #colSums矩阵或数组列的组合，qr是指QR分解，svd指奇异值分解
        Z <- Z[,vids]
        K <- K[vids,vids]
      }
	...
	#QR（正交三角）分解法是求一般矩阵全部特征值的最有效并广泛应用的方法，
    ...
    emma.eigen.R.wo.Z <- function(K, X) {
      ...
      stopifnot(!is.complex(eig$values))#stopifnot用于嵌入函数中, 在检测某一确定的变量或条件是否为真, 如果判断为假,则为终止程序
                     ...
                     esp=1e-10, eig.L = NULL, eig.R = NULL)
{
  ...
  return (list(REML=maxLL,delta=maxdelta,ve=maxve,vg=maxvg))
	...
	#第一步EMMA估计\lambda g 已经结束，开始估计\lambda \beta .
    #矩阵B的特征值分解  （10）
	...
	#线性变换后的模型 、\beta=0时的对数似然和限制对数似然函数情况（11）
                         ...
                         esp=1e-10, eig.L = NULL, eig.R = NULL)
{
  ...
  return (list(REML=maxLL,delta=maxdelta,ve=maxve,vg=maxvg,U_R=eig.R$vectors,etas.1=etas.1,etas=etas,lambda=eig.R$values))
    ...
    emma.ML.LRT.c.noalpha <- function(ys, xs, K=1, Z, X0, ngrids=100, llim=-10, ulim=10, esp=1e-10) {
      ...
      for (i in 1:t){
        ...
        if(length(MLE1$vg)!=0){
          ...
          ps[i]<-if(stats[i]<=1e-100) 1 else pchisq(stats[i],1,lower.tail=F)/2 #pchisq累积卡方密度
        }else{
          ps[i]<-1
        }
      }
    ...
    if(flagREMLE==1){
      ...
      registerDoParallel(cl)  #registerDoParallel、foreach计算机中的并行计算，可以提高代码的运行速率
      ...
      ff=foreach(i=1:t,.combine = 'rbind')%dopar%{
        ...
        if(length(MLE1$vg)!=0){
          ...
          var.bhats.ratio[i] <- MLE1$delta^2*temp%*%Lam%*%t(temp)%*%temp%*%Lam%*%t(temp)+MLE1$delta*temp%*%Lam1%*%t(temp)
          d[i] <- (1-var.bhats.ratio[i])*((1-var.bhats.ratio[i])>=0) 
          stats[i]<- 2*(MLE1$REML-MLE0$REML)  #似然比统计量LD  21
          ps[i]<-if(stats[i]<=1e-100) 1 else pchisq(stats[i],1,lower.tail=F)/2  #\chi方检验  22
        }else{
          ps[i]<-1 
        }
    ...
    colnames(parmsShow)<-c("maker","Chromosome","Marker position (bp)","p-value","SNP effect",
                           "QTN-to-residual variance ratio","QTN variance","Residual variance","MAF","r2 (%)")
    
    #return(parmsShow)
  
    Xemma<-data.frame(chr.locus=xnames,REML.LRT.c2.new)
    vid<-which(as.numeric(Xemma$ps)<=svpal) #与固定的p-value进行比较并记录
    ...
    if(length(vid)!=0){       #vid <- pval
      ...
      idslod<-which(emma.lod>=svmlod) #比较
